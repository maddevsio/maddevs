...
function buildMessage({ analytics, range, lighthouse }) {
  ...
  return [
    header(`${getEmoji('calendar', 'telegram')} Отчет радиатора по ключевым метрикам за ${range.text}`),
    divider(),
    section(`За отчетный период сайт https://maddevs.io посетило *${core.users.value} пользователей*. Всего *${core.sessions.value} сессий*, средняя длительность 1 сессии составляет *${core.duration.value}*. *${core.bounceRate.value}%* пользователей закрыли сайт никак с ним не провзаимодействовав.`),
    section(list(
